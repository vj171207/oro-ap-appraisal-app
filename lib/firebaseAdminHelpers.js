...
// This is a third, independent implementation of "fetch config/auditors and
// drop malformed entries" — js/auditorList.js's loadAuditorList() and
// js/settings-calibration.js both do the same filtering client-side, via
// the Firestore JS SDK's plain-object format. This one can't structurally
// share code with those two, since it runs server-side against Firestore's
// raw REST API (the verbose {stringValue}/{arrayValue} field-wrapper format
// below), a completely different shape from what the client SDK returns.
// If the filtering rule itself ever changes (e.g. requiring a new field),
// remember there are three places, not one.
